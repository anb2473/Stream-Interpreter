...
def main():
    ...
    print(f'{FG_BRIGHT_CYAN}{time.time() - start_time}{RESET}: {FG_BRIGHT_GREEN}Successfully executed file{RESET}'
          f' at \'{f}\': \n{FG_BRIGHT_BLUE}{getattr(ExecutionPipeline, ext[1:])(f, return_type, params)}{RESET}')
...
def handler(f: str, return_type: str, params: dict):
    ...
    _, ext = os.path.splitext(f)
    # SEARCH EXECUTION PIPELINE FOR PROPER FUNCTION HANDLER FOR FILE TYPE
    return getattr(ExecutionPipeline, ext[1:])(f, return_type, params)
        ...
        if value.startswith('exec'):
            ...
            if not self.calls.__contains__(value[5:].split('(')[0]):
                print(f'{FG_BRIGHT_CYAN}{time.time() - start_time}{RESET}: {BOLD}{FG_BRIGHT_RED}'
                      ...
                      f'{ITALIC}\'{value[5:].split('(')[0]}\'{RESET}, {FG_BRIGHT_BLUE}line {line_num + 1}{RESET}')
                sys.exit(1)
            ...
            try:
                ...
                try:
                    if type(obj) is dict:
                        ...
                        value = obj[self.evaluate(value.split('.')[1], 'int', local, line_num)]
                except IndexError:
                    print(f'{FG_BRIGHT_CYAN}{time.time() - start_time}{RESET}: {BOLD}{FG_BRIGHT_RED}'
                          f'Index Error:{RESET} {ITALIC}{obj}.'
                          f'{self.evaluate(value.split('.')[1], 'int', local, line_num)} '
        ...
        while i < len(split_file):
            ...
            if line.startswith('let'):
                ...
                self.calls[split_line[0]] = [split_line[1].split('->')[0].strip()[1:-1].replace('+', ' '),
                                             ...
                                             [(param, param_type) for value in split_line[1].split('(')[1].split(',')
                ...
                return self.evaluate(line.split(' ')[1].strip(), return_type, local, i)
            i += 1
        print(
            f'{FG_BRIGHT_CYAN}{time.time() - start_time}{RESET}: {FG_BRIGHT_GREEN}'
            f'Program finished with local data{RESET}:'
